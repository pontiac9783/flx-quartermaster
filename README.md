...
| **Tab tills** | A member's balance is split **bar** vs **merch**. Payments are tagged to one of them: bar payments land in the open bar shift's cash box, merch payments go to the Quartermaster. **Pay both** settles the two in one go. |
...
| **Receiving** | One **📦 Receive** screen: tap-to-add list (low stock first) or type-and-Enter search, qty steppers, unit cost per line, a supplier note, and **Paid by**: treasurer (cost only), cash drawer (bar box or merch — logged as a cost out of that till), or personal (reimbursement list). Saved as one batch; spirits entered in bottles. |
